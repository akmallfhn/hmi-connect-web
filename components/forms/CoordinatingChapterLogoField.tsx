...
async function uploadCoordinatingChapterLogo(file: File, fileExt: string) {
  const filePath = `coordinating-chapters/${Date.now()}.${fileExt}`;
...
interface CoordinatingChapterLogoFieldProps {
  imageUrl: string;
...
// Mirrors BranchLogoField/ChapterLogoField — same square rounded-lg preview + upload/remove pair, just uploading into the coordinating-chapters/ storage folder.
export default function CoordinatingChapterLogoField({
  imageUrl,
  onChange,
  onUploadingChange,
  disabled,
  size = 80,
  layout = "row",
}: CoordinatingChapterLogoFieldProps) {
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const publicUrl = await uploadCoordinatingChapterLogo(
        compressed,
        compressedExt
      );
      onChange(publicUrl);
    } catch (error) {
      console.error("[CoordinatingChapterLogoField] logo upload threw:", error);
      if (isStoragePolicyError(error)) {
        toast.error(
          "Upload logo ditolak Supabase. Izinkan folder coordinating-chapters di bucket hmi-connect."
        );
      } else {
