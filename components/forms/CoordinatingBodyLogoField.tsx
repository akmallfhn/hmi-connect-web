...
async function uploadCoordinatingBodyLogo(file: File, fileExt: string) {
  const filePath = `coordinating-bodies/${Date.now()}.${fileExt}`;
...
interface CoordinatingBodyLogoFieldProps {
  imageUrl: string;
...
export default function CoordinatingBodyLogoField({
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const publicUrl = await uploadCoordinatingBodyLogo(
        compressed,
        compressedExt
      );
      onChange(publicUrl);
    } catch (error) {
      console.error("[CoordinatingBodyLogoField] logo upload threw:", error);
      if (isStoragePolicyError(error)) {
        toast.error(
          "Upload logo ditolak Supabase. Izinkan folder coordinating-bodies di bucket hmi-connect."
        );
      } else {
