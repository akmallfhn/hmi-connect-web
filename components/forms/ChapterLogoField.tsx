...
async function uploadChapterLogo(file: File, fileExt: string) {
  const filePath = `chapters/${Date.now()}.${fileExt}`;
...
// Mirrors BranchLogoField/CoordinatingBodyLogoField — same square rounded-lg preview + upload/remove pair, just uploading into the chapters/ storage folder.
export default function ChapterLogoField({
  imageUrl,
  onChange,
  onUploadingChange,
  disabled,
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const publicUrl = await uploadChapterLogo(compressed, compressedExt);
      onChange(publicUrl);
    } catch (error) {
      console.error("[ChapterLogoField] logo upload threw:", error);
      if (isStoragePolicyError(error)) {
        toast.error(
          "Upload logo ditolak Supabase. Izinkan folder chapters di bucket hmi-connect."
        );
      } else {
