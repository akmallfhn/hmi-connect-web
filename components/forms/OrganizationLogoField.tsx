...
async function uploadOrganizationLogo(file: File, fileExt: string) {
  const filePath = `organizations/${Date.now()}.${fileExt}`;
...
// Mirrors CoordinatingBodyLogoField, uploading into the organizations/ folder instead.
export default function OrganizationLogoField({
  imageUrl,
  onChange,
  onUploadingChange,
  disabled,
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      console.error("[OrganizationLogoField] logo upload threw:", error);
      if (isStoragePolicyError(error)) {
        toast.error(
          "Upload logo ditolak Supabase. Izinkan folder organizations di bucket hmi-connect."
        );
      } else {
