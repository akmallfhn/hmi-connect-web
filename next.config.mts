...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      ...
      {
        source:
          "/:path((?!auth/login|auth/forget-password|reset-password/.*|api/auth/callback/google|profile/.*|trainings(?:/.*)?|invitations/.*|feeds/.*|_next/static|_next/image|favicon\\.ico|sitemap\\.xml|robots\\.txt|.*\\..*).*)",
        has: [
          {
            type: "header",
            key: "host",
